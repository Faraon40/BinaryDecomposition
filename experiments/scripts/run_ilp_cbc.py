...
- experiments/results/ilp_baseline/visualizations/
- experiments/results/ilp_baseline/csv/
"""
...
def run_ilp_baseline(
    ...
    ----------
    image_dir_name : str
        Directory name under data/datasets/ (e.g., "validation", "synthetic").
    ...
    print("=" * 70)
    print(f"ILP BASELINE SOLVER - {mode_str}")
    print("=" * 70)
    ...
    for i, img_path in enumerate(image_paths, 1):
        ...
        try:
            ...
            if rectangles:
                ...
                # Save rectangles to JSON (always save ILP results)
                image_stem = Path(img_path.name).stem
                rect_filename = f"{image_stem}_ilp_{len(rectangles)}_rects.json"
                rect_path = rect_dir / rect_filename

                data = {
                    "image_name": img_path.name,
                    "algorithm": "ilp_baseline",
                    "optimal": True,
                ...
                # Save visualization
                viz_filename = f"{image_stem}_ilp_{len(rectangles)}_rects.png"
    ...
    with open(csv_path, 'w', newline='', encoding='utf-8') as f:
        fieldnames = ['image', 'size', 'pixels', 'rectangles', 'candidates',
                     'solve_time', 'enum_time', 'ilp_time', 'status']
    ...
    print("SUMMARY")
    print("=" * 70)
    print(f"{'Image':<30} {'Size':<10} {'Rects':<8} {'Time':<10} {'Status':<12}")
    print("-" * 70)

    for r in results:
        print(
            f"{r['image']:<30} {r['size']:<10} {r['rectangles']:<8} "
            f"{r['solve_time']:<10.2f} {r['status']:<12}"
        )
